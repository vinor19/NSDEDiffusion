...
class NSDE(BaseNSDE):
    ...
    def drift_moments(self, m, P):
        ...
        Returns:
            ...
            - P_nxt (torch.FloatTensor): output covariance with (batch_size, d_in, d_in)
        """
        jac = self.f1.jac(m, P).to("cuda")
        m_nxt, P_nxt = self.f1.next_moments(m, P)
            
        jac = self.relu.jac(m_nxt, P_nxt).to("cuda")@jac
        m_nxt, P_nxt = self.relu.next_moments(m_nxt, P_nxt)
        
        jac = self.dropout.jac(m_nxt, P_nxt).to("cuda")@jac
        m_nxt, P_nxt = self.dropout.next_moments(m_nxt, P_nxt) 
        
        jac = self.f2.jac(m_nxt, P_nxt).to("cuda")@jac
        ...
        jac = self.f3.jac(m_nxt, P_nxt).to("cuda")@jac
    ...
    def diffusions_moments(self, m, P):
        """
        Diagonal diffusion function in moment matching mode.
